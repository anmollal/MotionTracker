...
public class LeftIndexActivity extends AppCompatActivity {


    List<LeftIndexActivity.ScreenLoc> swipe;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Button button = (Button) findViewById(R.id.lindex_btn);
        button.setOnClickListener(new View.OnClickListener() {
            public void onClick(View v) {
                // Code here executes on main thread after user presses button
                startActivity(new Intent(LeftIndexActivity.this, FinalActivity.class));
            }
        });
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        swipe.add(new LeftIndexActivity.ScreenLoc(ev.getX(0), height - ev.getY(0)));
        if (ev.getAction() == MotionEvent.ACTION_UP) {
            endTime = System.currentTimeMillis();
            File file = new File(this.getExternalFilesDir("SwipeData"), "LeftIndexSwipeData.csv");
            try {
                file.createNewFile();
    ...
    private String swipeString(List<LeftIndexActivity.ScreenLoc> list) {
        //figure out fragments and uncomment this after
        //String str = String.format("%s, %s", currentState, Long.toString(endTime - startTime));
        String str = String.format("%s %s","Left Index", Long.toString(endTime - startTime));
